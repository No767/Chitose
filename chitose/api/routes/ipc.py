...
async def create_tag(request: CreateTag, background_task: BackgroundTasks) -> CreateTagResponseModel:
    background_task.add_task(createTag, path="create_tag", data=request)
    return CreateTagResponseModel(status=202, message="Successfully created tag", data=request)
